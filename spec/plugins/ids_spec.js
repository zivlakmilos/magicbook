  ...
  it("should add ids to sections without id", function(done) {
    var uid = triggerBuild({
      ...
      finish: function() {
        ...
        expect($('section[data-type=sect1]').eq(1).attr('id')).toEqual("sect-1-vqWMCPD");
        expect($('section[data-type=sect5]').eq(0).attr('id')).toEqual("sect-5-RDyxCZa");
        done();
      }
    });
  });

  it("should not have duplicate ids for same structure in separate files", function(done) {
    var uid = triggerBuild({
      builds: [{ format: "html" }],
      files: [
      ...
      finish: function() {
        var content1 = buildContent(uid, "build1/duplicate_id1.html").toString();
        var content2 = buildContent(uid, "build1/duplicate_id2.html").toString();
