...
var Plugin = function(registry) {
  registry.after("load", "navigation:placeholders", this.insertPlaceholders);
  registry.after("layouts", "navigation:insert", this.insertNavigation);
};
...
Plugin.prototype = {
  ...
  insertNavigation: function(config, stream, extras, callback) {
    stream = stream.pipe(
      through.obj(function(file, enc, cb) {
        ...
        if (file.prev) {
          changed = true;
          file.prev.$el =
            file.prev.$el || cheerio.load(file.prev.contents.toString());
          var title = file.prev
            .$el("h1")
            .first()
            .text();
          contents = contents.replace(/MBINSERTPREVLABEL/g, title);
          var href = path.relative(
        ...
        if (file.next) {
          changed = true;
          file.next.$el =
            file.next.$el || cheerio.load(file.next.contents.toString());
          var title = file.next
            .$el("h1")
            .first()
            .text();
