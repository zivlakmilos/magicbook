...
module.exports = function(jsonConfig) {
  ...
  _.each(jsonConfig.builds, function(build, i) {
    ...
    if (!build.format)
      return console.log(
        "no format in build setting. Skipping build " + (i + 1)
      );
    ...
    var destination = helpers.destination(
      config.destination,
      config.buildNumber
    );
    ...
    executer.execute(config.plugins, config.disablePlugins, args, function(
      config,
      stream,
      extras
    ) {
